...
#include "broadcast-retx-strategy.hpp"
...
NFD_LOG_INIT("BroadcastRetxStrategy");

namespace nfd {
namespace fw {

  //const Name BroadcastStrategy::STRATEGY_NAME("ndn:/localhost/nfd/strategy/broadcast/%FD%02");
NFD_REGISTER_STRATEGY(BroadcastRetxStrategy);

BroadcastRetxStrategy::BroadcastRetxStrategy(Forwarder& forwarder,const Name& name): Strategy(forwarder)
{
...
*/
  ...
  if (!parsed.parameters.empty()) {
    BOOST_THROW_EXCEPTION(std::invalid_argument("BroadcastRetxStrategy does not accept parameters"));
  }
  if (parsed.version && *parsed.version != getStrategyName()[-1].toVersion()) {
    BOOST_THROW_EXCEPTION(std::invalid_argument(
                                                "BroadcastRetxStrategy does not support version " + std::to_string(*parsed.version)));
  }
...
const Name&
BroadcastRetxStrategy::getStrategyName()
{
  static Name strategyName("/localhost/nfd/strategy/broadcastretx/%FD%02");
  return strategyName;
}
  
void BroadcastRetxStrategy::energyTracer(int nodeId, int hopCount, double nodeCurrentPowerA)
{
    std::shared_ptr<std::ofstream> os(new std::ofstream());
...
void
BroadcastRetxStrategy::afterReceiveInterest(const Face& inFace, const Interest& interest,const shared_ptr<pit::Entry>& pitEntry)
{
...
//          std::cerr << "current Time:"<<ns3::Simulator::Now().ToDouble(ns3::Time::MS)<<"\n";
          ...
          ns3::Simulator:: Schedule (ns3::Seconds(0), &BroadcastRetxStrategy::energyTracer, this, (int)node->GetId (), hopCount, m_nodeCurrentPowerA);
          }
